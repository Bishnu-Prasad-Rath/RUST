...
pub fn scan_and_enforce(&mut self, threshold_secs : u64)->Vec<u32>{
  ...
  self.frozen_since.retain(|pid, _| current_frozen.contains(pid));
  ...
  for pid in current_frozen{
    ...
    if entry.elapsed() >= Duration::from_secs(threshold_secs){
      println!("\n [WATCH-DOG ALERT] PID {} unresponsive for > {}s! Force Terminating...",pid,threshold_secs);
...
fn get_all_hung_pids()->Vec<u32>{
  ...
  unsafe{
    ...
    EnumWindows(Some(enum_window_callback), &mut hung_pids as *mut _ as isize);
  }
  hung_pids
}
...
pub fn emergency_kill_foreground_app() {
    unsafe {
        ...
        if hwnd != 0 {
            ...
            if pid > 0 {
                println!("\n⚡ [EMERGENCY PANIC HOTKEY TRIGGERED] Terminating foreground PID: {}", pid);
                force_kill_pid(pid);
            }
        }
    }
