...
use crate::system::{kill_process, ask_user_confirmation};

pub fn scan_process_threats(proc : &mut ProcessInfo){
  let name_lower = proc.name.to_lowercase();

  let suspicious_keywords = ["mimikatz", "keylogger", "netcat", "psexec", "hacktool", "miner"];
  ...
  if name_lower == "powershell.exe" || name_lower == "cmd.exe" || name_lower == "wscript.exe"{
      if proc.memory_mb > 300 {
        let reason = format!("High-memory script execution environment ({}MB)",proc.memory_mb);
      ...
      if let HealthStatus::SecurityRisk(ref threat) = proc.status{
        match threat{
          ThreatLevel::Critical(reason) | ThreatLevel::Medium(reason) => {
                    ...
                    println!("   └─ Name: {} | PID: {} | Reason: {}", proc.name, proc.pid, reason);

                    let prompt = format!("   └─ Do you want to terminate security threat PID {} ({})?", proc.pid, proc.name);
        ...
        if proc.category == ProcessCategory::UserApp {
            if let HealthStatus::HighMemoryUsage(mem) = proc.status {
                if mem >= memory_threshold_mb {
                    ...
                    println!("   └─ Name: {} | PID: {} | Memory: {} MB (Limit: {} MB)", proc.name, proc.pid, mem, memory_threshold_mb);

                    // Ask user for confirmation instead of auto-killing
                    let prompt = format!("   └─ Do you want to terminate PID {} ({})?", proc.pid, proc.name);
            ...
            if let HealthStatus::HighMemoryUsage(mem) = proc.status {
                println!("\n🛡️ SYSTEM ALERT: High memory on system process {} ({} MB). Automatic kill disabled for safety.", proc.name, mem);
            }
        }
    }
}